...
describe('knobsHash determinism + cross-mode separation (CDX-4)', () => {
  ...
  test('#3515: detail set vs unset produces DIFFERENT hashes (cache contamination prevention)', () => {
    ...
    // Bumped 18→19 because preferred-type coverage now changes post-fusion
    expect(KNOBS_HASH_VERSION).toBe(19);
  });
...
describe('v0.42.3.0 — autocut knobs', () => {
  test('KNOBS_HASH_VERSION is 19 (15→16 detail fold #3515; 16→17 degradation-stamp epoch; 17→18 preferred-type recall; 18→19 preferred-type coverage)', () => {
    expect(KNOBS_HASH_VERSION).toBe(19);
  });
