...
describe('parseEtimeToMs (PID-reuse guard, timezone-free)', () => {
  // The guard used to derive a start time from `ps -o lstart=`, which prints
  // LOCAL time with no timezone ("Sat Jul 25 23:17:08 2026"). Date.parse reads
  // that as UTC, so on any host east of UTC the computed start time ran ahead
  // of the registration timestamp by the whole offset — +8h on this brain —
  // and the guard dropped every live worker. `ps -o etime=` is elapsed time,
  // so it carries no timezone at all. (`etimes` is Linux-only; `etime` is
  // portable to macOS.)
  ...
  test('returns null for undeterminable input (callers must not read null as reused)', async () => {
    const { parseEtimeToMs } = await reg();
