...
/** Seed ordinary mail plus one allowed and one out-of-scope weekly page. */
...
describe('hybridSearch preferred-type recall', () => {
  ...
  test('an ordinary exact alias stays rank 1 while preferred weekly coverage stays visible', async () => {
    const allowed = await seedMarketCorpus();
    await seedPage({
      sourceId: 'bulk-mail',
    ...
    expect(results[0]?.slug).toBe('notes/ordinary-market-alias-owner');
    expect(results[0]?.alias_hit).toBe(true);
    expect(results[1]?.slug).toBe('reports/freight-digest-2026-w31');
    expect(embedCalls).toBe(1);
  });

  test('an ordinary exact title stays rank 1 while preferred weekly coverage stays visible', async () => {
    const allowed = await seedMarketCorpus();
    await seedPage({
      sourceId: 'bulk-mail',
      slug: 'notes/market-last-week',
      type: 'note',
      title: 'Market Last Week',
      body: 'A named operating note whose identity exactly matches the lookup.',
    });

    const results = await hybridSearch(engine, 'market last week', searchOpts(allowed));

    expect(results[0]?.slug).toBe('notes/market-last-week');
  ...
  test('private preferred candidates are never re-admitted for an excluding caller', async () => {
    ...
    await seedPage({
      sourceId: 'derived-weekly',
      slug: 'reports/private-market-weekly',
      type: 'market-weekly',
      title: 'Private Market Weekly',
      body: 'what happened in the market last week '.repeat(20),
    ...
    expect(results.map((result) => result.slug)).toContain('reports/freight-digest-2026-w31');
    expect(results.map((result) => result.slug)).not.toContain('reports/private-market-weekly');
  });
  ...
  test('typed vector pool exhaustion does not pollute ordinary vector telemetry', async () => {
    const weekly = syntheticResult('reports/preferred-weekly', 'market-weekly', 100);
    const { restore } = installSyntheticSearchArms([weekly]);
    const syntheticSearchVector = engine.searchVector.bind(engine);
    let meta: HybridSearchMeta | undefined;
    engine.searchVector = async (embedding, opts) => {
      if (opts?.types?.join(',') === 'market-weekly') {
  ...
  test('the final relational evidence slot composes after preferred coverage', async () => {
    await seedPage({
      sourceId: 'default',
      slug: 'companies/market-last-week',
      type: 'company',
      title: 'Market Last Week',
    ...
    await engine.addLink(
      ...
      'companies/market-last-week',
      '',
      'invested_in',
      'manual',
    );
    const weekly = syntheticResult('reports/preferred-weekly', 'market-weekly', 100, 'default');
    const { restore } = installSyntheticSearchArms([weekly]);
    let meta: HybridSearchMeta | undefined;
    ...
    try {
      const results = await hybridSearch(
        engine,
        'who invested in market last week',
        {
          ...searchOpts(['default']),
          limit: 3,
