...
- `test/page-list-filters.test.ts` — strict validation for non-negative offsets, supported frontmatter operators, top-level field names, clause/value/projection caps, sparse arrays, and response projection.
- `test/list-pages-enumeration.test.ts` — in-memory PGLite exact enumeration: deterministic slug pagination, structured filter semantics, literal special characters, and combined source/prefix/frontmatter scope.
- `test/list-pages-operation.test.ts` — operation plumbing for exact-enumeration inputs while preserving unqualified federation, trusted-local limits, remote caps, source grants, and the plain-array response.
- `test/cli-args.test.ts` — operation CLI parsing for numbers and JSON array/object values, including named malformed-JSON and wrong-container failures.
...
- `test/e2e/list-pages-regression.test.ts` — PGLite back-compat for the existing listing shape, default ordering, `updated_after`, the sort enum, and invalid-sort fallback.
- `test/e2e/list-pages-enumeration-postgres.test.ts` — real-Postgres counterpart for complete stable pagination, structured filter semantics, literal special-character handling, source/prefix composition, and non-string frontmatter rejection.
