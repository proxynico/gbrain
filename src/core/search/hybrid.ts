...
export async function hybridSearch(
  ...
  const searchOpts: SearchOpts = {
    ...
    // v0.33: page-type filters, including multi-type whoknows
    // ('person','company'). Push to SQL level so the limit budget goes to
    // candidate-typed pages instead of being eaten by other page types.
    // PGLite keyword/vector arms consume `types`; mirror a scalar into that
    // shared field while preserving Postgres' scalar `type` contract.
    type: opts?.type,
